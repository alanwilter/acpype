...
def params_update(cparams, aparams, newattyp, imp_rearg):
    ...
    for bond in aparams.bond_types.keys():
        i = bond[0]
        j = bond[1]
        if newattyp == 0:
            ni = i
            ...
            ni = get_new_attype(i)
            nj = get_new_attype(j)
        if min((ni, nj), (nj, ni)) not in cparams.bond_types.keys():
            cparams.bond_types[min((ni, nj), (nj, ni))] = aparams.bond_types[bond]

    for angle in aparams.angle_types.keys():
        i = angle[0]
        j = angle[1]
        k = angle[2]
        if newattyp == 0:
            ...
            nj = get_new_attype(j)
            nk = get_new_attype(k)
        if min((ni, nj, nk), (nk, nj, ni)) not in cparams.angle_types.keys():
            cparams.angle_types[min((ni, nj, nk), (nk, nj, ni))] = aparams.angle_types[angle]

    for dih in aparams.dihedral_types.keys():
        i = dih[0]
        j = dih[1]
        k = dih[2]
        l = dih[3]
        if newattyp == 0:
            ...
            nk = get_new_attype(k)
            nl = get_new_attype(l)
        if min((ni, nj, nk, nl), (nl, nk, nj, ni)) not in cparams.dihedral_types.keys():
    ...
    for imp in aparams.improper_periodic_types.keys():
        i = imp[0]
        j = imp[1]
        k = imp[2]
        l = imp[3]
        ...
        if imp_rearg == 1:
            ...
            attyp = [ni, nj, nk, nl]
            attyp1 = [ni, nj, nl]
            xlist = [attyp[i] for i in xrange(4) if (attyp[i] == 'X')]
            rattyp1 = [attyp1[i] for i in xrange(3) if (attyp1[i] != 'X')]
    ...
    for nbfix in aparams.nbfix_types.keys():
        ...
        if newattyp == 0:
            ...
            ni = get_new_attype(i)
            nj = get_new_attype(j)
        if min((ni, nj), (nj, ni)) not in cparams.nbfix_types.keys():
