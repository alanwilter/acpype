...
group.add_argument('-i', dest='mdins', metavar='FILE', nargs='*',
                   ...
                   solve=REDOX, temp0=TEMPERATURE. Also, each reference mdin file must contain ig=RANDOMNUM. The reference mdin
...
def main(opt):
    ...
    # Checking on the mdin file(s) in the required pointer are there
    mdinlines = []
    for i in range(nrefgrpfls):
        ...
        for dim in range(ndims+1):
            ...
            if (not FoundFlag[dim] and txt.lower() != "hamiltonian" and txt.lower() != "hremd"):
                ...
                elif (FoundHamiltonian[hdims[hdims.index(dim)]] and FoundFlag[dim]):
                    ErrorExit('Pointer %s was found on both groupfile %s and on mdin file %s but it can only be stated once'%(flag,opt.groupfiles[i],opt.mdins[i]))
        mdin_file.close()
    del mdin_file, flag
