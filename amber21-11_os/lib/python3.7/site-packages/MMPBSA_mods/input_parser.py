...
"""
from MMPBSA_mods.exceptions import InputError, InternalError
      ...
      for i in range(len(namelist_fields)):
         for j in range(len(namelist_fields[i])):
            ...
            if not '=' in namelist_fields[i][j]:
               ...
               found = False
               for key in list(self.namelists[declared_namelists[i]].\
                           variables.keys()):
...
input_file.addNamelist('general', 'general',
  ...
  ['entropy', int, 0, 'Do quasi-harmonic entropy calculation'],
  ...
  ['receptor_mask', str, None,
     'Amber mask of receptor atoms in complex prmtop.'],
