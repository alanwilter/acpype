...
from MMPBSA_mods.exceptions import PrmtopError, SelectionError
...
class MMPBSA_System(object):
   """
   Sets up a system of a complex, receptor, and ligand. The prmtops get 
   initialized to prmtop objects immediately, so they must exist. Use it as:
   ...
   def __init__(self, complex_prmtop, receptor_prmtop=None, ligand_prmtop=None):
      """ Initializes the topology file classes for all of the prmtop files """
   ...
   def _binding_mask(self, selection, in_complex):
      ...
      while i < len(self.res_list):
         ...
         if rec_start != -1:
            if rec_start == rec_end: receptor_mask += ',%d' % rec_start
            else: receptor_mask += ',%d-%d' % (rec_start, i-1)
            rec_start = -1
         if lig_start != -1:
            if lig_start == lig_end: ligand_mask += ',%d' % lig_start
            else: ligand_mask += ',%d-%d' % (lig_start, i-1)
            lig_start = -1
         ...
         if self.res_list[i].receptor_number != None: # it's in receptor
            ...
            else: receptor_selection[self.res_list[i].receptor_number-1] = 1
            in_rec = True
         elif self.res_list[i].ligand_number != None: # it's in ligand
            if in_complex: lig_start = i+1
            else: ligand_selection[self.res_list[i].ligand_number-1] = 1
            in_rec = False
      ...
      if in_complex:
         if rec_start != -1:
            if in_complex: rec_end = len(self.res_list)
            else: rec_end = self.res_list[len(self.res_list)-1].receptor_number
            receptor_mask += ',' + range_string(rec_start, rec_end)

         if lig_start != -1:
            if in_complex: lig_end = len(self.res_list)
            else: lig_end = self.res_list[len(self.res_list)-1].ligand_number
            ligand_mask += ',' + range_string(lig_start, lig_end)
      else:
         # Get receptor mask
         i = 0
         ...
         while i < len(receptor_selection):
            if receptor_selection[i] == 1:
               ...
               while i < len(receptor_selection) and receptor_selection[i] == 1:
                  i += 1
               if start == i: receptor_mask += ',%d' % start
               else: receptor_mask += ',%d-%d' % (start, i)
               start = -1
            i += 1

         if start != -1:
            # The end was selected
            if start == len(receptor_selection): receptor_mask += ',%d' % start
            else: receptor_mask += ',%d-%d' % (start, len(receptor_selection))
         ...
         while i < len(ligand_selection):
            if ligand_selection[i] == 1:
               ...
               while i < len(ligand_selection) and ligand_selection[i] == 1:
                  i += 1
               if start == i: ligand_mask += ',%d' % start
               else: ligand_mask += ',%d-%d' % (start, i)
               start = -1
            i += 1

         if start != -1:
            # The end was selected
            if start == len(ligand_selection): ligand_mask += ',%d' % start
            else: ligand_mask += ',%d-%d' % (start, len(ligand_selection))
